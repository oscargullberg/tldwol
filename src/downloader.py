...
async def _download_file(url, file_path=None):
    file_path = file_path or Path(DOWNLOAD_DIR) / _md5_encode(url)
...
async def _download_youtube_audio(url):
...
async def _download_apple_podcast(url):
    async with aiohttp.ClientSession() as session:
        async with session.get(yarl.URL(url, encoded=True)) as response:
            ...
            if match:
                ...
                return await _download_file(episode_download_url)
...
DOWNLOAD_STRATEGIES = [
    DownloadStrategy(
        ...
        handler=_download_youtube_audio,
    ),
    DownloadStrategy(
        test=lambda url: re.match(
            r"(https?://)?podcasts\.apple\.com/\w+/podcast/[\w-]+/id\d+\?i=\d+", url
        ),
        handler=_download_apple_podcast,
    ),
    DownloadStrategy(
        test=lambda url: re.match(r"https?://.+", url), handler=_download_file
    ),
]
